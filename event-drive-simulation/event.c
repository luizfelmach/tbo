...
Event *create_event(double t, Particle *a, Particle *b) {
    ...
    e->b = b;
    if (a != NULL)
        e->count_A = hit_count(a);
    else
        e->count_A = -1;
    if (b != NULL)
        e->count_B = hit_count(b);
    else
        e->count_B = -1;
 ...
 */
int compare(Event *e, Event *f) { return (int)(e->time - f->time); }
